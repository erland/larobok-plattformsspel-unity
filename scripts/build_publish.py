...
def postprocess_epub(epub: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="plattformsspel-unity-epub-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="plattformsspel-unity-pdf-") as tmp_name:
        tmp = Path(tmp_name)
